...
- **Repo-local identity overrides the global one.** `~/.gitconfig` holds `sathviknookala@neuralads.ai`;
  ...
  local config, will silently carry the work address.
...
unmodified at commit `c377be1` and no longer exists in the tree, so there is one copy of each claim.
...
- **Two GitHub identities are live on this machine and neither default is the right one.**
  ...
  check `git config user.email` before committing. The SSH key authenticates as `NeuralNookala`, so
  **do not switch `origin` to an SSH URL**.
