...
- Not a git repository yet. `git init` before the first real change; record the default branch name
  and remote here once they exist. Global identity is `~/.gitconfig` — check it is the intended one
...
- **The user's global git identity applies.** Confirm `git config user.email` is the intended
  address before the first commit; this machine has more than one context in use.
