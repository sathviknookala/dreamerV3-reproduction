...
| 3 | **AC** | Actor estimator | v2: Reinforce for both action types, baseline `v_ψ(s_t)` | Same as v2 | v1 used pathwise backprop for continuous actions — a different objective for these tasks | None relative to v2. Recorded because silently using v1's rule is the most likely single error, and it would not raise an exception. **Verified at M8**: the pathwise route is not merely unused, it is unavailable — `imagine_trajectory` builds under `no_grad` and the actor loss raises on features that carry a graph. |
...
| 6 | **RR** | Replay capacity | 5e6 | **5e5** | 124 GiB RAM; 5e6 × 64×64×3 uint8 ≈ 61 GB for images alone before any other field | **The inertness expectation was wrong and is refuted (§10-11).** 5e5 < 1e6, so a Walker run evicts its first half, whole episodes at a time, from roughly the half-way point; only Cartpole's 5e5 budget is inert. Walker's behaviour updates therefore never see the early exploratory data again. Measured 2026-09-20. |
...
| 5 | Random-policy return floor | M1 | **VALIDATED 2026-09-20.** 20 complete 1000-step episodes per task, uniform random over the action bounds, action repeat 1. **Walker 32.21 ± 4.37** (min 27.44, max 47.34); **Cartpole 24.17 ± 15.92** (min 4.95, max 63.90) — [`random-floor-walker-2026-09-20.json`](../results/m1/random-floor-walker-2026-09-20.json), [`random-floor-cartpole-2026-09-20.json`](../results/m1/random-floor-cartpole-2026-09-20.json). Cartpole's floor is noisy enough that its `return_max` of 63.90, not its mean, is the honest bar. |
...
| 9 | Per-stage time share | M9 | **VALIDATED 2026-09-19.** Full decomposition of a complete update at H = 5/15/30 — [`profile-walker-2026-09-19.csv`](../results/m9/profile-walker-2026-09-19.csv). Steady-state shares of a Walker H=15 run: backward+optimizer **42.7%**, world-model forward/loss 26.5%, collection+render 15.8%, imagination+behaviour 7.6%, evaluation 3.7%, checkpointing 2.3%, episode rebuild 0.6%, replay transfer 0.3%. The `unattributed_ms` residual against a separately timed update is **under 1%**, which is what says the decomposition accounts for the update. |
| 10 | Realized training ratio | M9 | **VALIDATED 2026-09-20.** `realized_train_ratio` is a column of both pilot logs and reads exactly **64.0**, the requested value, at every row of both full-budget runs — [`results/m9/pilot/`](../results/m9/pilot/). The §7.7 convention is loss-bearing positions per collected transition: Walker 63,679,488 train positions over 1,000,000 control steps. |
| 11 | Replay RAM occupancy | M9 | **VALIDATED 2026-09-20 — §9-6's inertness hypothesis is REFUTED for Walker and confirmed for Cartpole.** Walker collected 1,000 episodes / 1,000,000 transitions and ended holding **500,000 / 500 complete episodes**: eviction triggered near the half-way point and the second half of the run evicted the first. Cartpole's 500,000-step budget exactly fills the buffer, so nothing is evicted and the reduction is inert there. Run manifests in [`results/m9/pilot/`](../results/m9/pilot/). Size at full occupancy is **12,334 bytes/transition, 5.744 GiB** serialized — [`checkpoint-cost-2026-09-19.json`](../results/m9/checkpoint-cost-2026-09-19.json); that is file size, not a measured resident-set figure. |
| 12 | Learning above the random floor | M9 gate | **Floor now measured (§10-5), and both pilots clear it** — [`results/m9/pilot/`](../results/m9/pilot/). Walker's final checkpoint is **16.1×** its floor and its last ten evaluations average 13.8×; Cartpole's is **5.5×**, with all 20 evaluations above the floor mean and 19 above the best random episode. **Walker's improvement persists; Cartpole's does not consolidate** — it plateaus, spikes three times and decays. Whether that satisfies "persisting beyond a transient spike" is the open half of this check. The qualification controls ([`results/m9/controls/`](../results/m9/controls/)) add that both final checkpoints beat an untrained agent, a zero-action policy and a uniform-random policy on **20/20 paired episodes** at matched initial conditions — but those are end-of-run measurements and do not speak to persistence. |
...
| **Online loop, checkpoint/resume, evaluation policy (§7.6, §7.8, §8.1)** | **`implemented`**, and the integration gate passes 75/75 on each task with 22/22 mutants killed ([`results/m9/`](../results/m9/)). **Not `validated`:** M9's gate is empirical — both pixel tasks must show improvement persisting beyond a transient spike, and Cartpole's does not consolidate ([`results/m9/pilot/`](../results/m9/pilot/)). Two full-budget pilots have run; `scripts/m9_gate.py` is implementation validation, not the gate. |
| §10-7, every module in §4.11 | **measured** — 686,846 trainable, `slowval` mirror excluded |
| §10-1…5, §10-8…11 | **measured** — toolchain and environment ([`m1/`](../results/m1/)), the random floor, H=30 peak VRAM, the per-stage profile, the realized training ratio and replay occupancy ([`m9/`](../results/m9/)) |
| §10-6 environment throughput | **`unmeasured`** — the last M1 deferral outstanding |
| §10-12 learning above the floor | **partially measured** — the floor exists and both pilots clear it; the persistence half is open |
| §10-13 final returns, curve areas, prediction error, cost | **`unmeasured`** — M10, not started |
