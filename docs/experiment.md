...
- Keep actor/critic reductions normalized consistently across horizons. Record the effective weighted sample counts so a longer horizon does not inadvertently multiply the loss scale.
  > **As implemented at M8, the first half of this control holds by construction and the second does not.** Both the actor and the critic reduce with an ordinary position mean over the `(N, H)` grid, never a sum and never a division by the summed weight, so the loss *scale* does not multiply with `H`. But the position weight is `cumprod(con)`, which decays along the rollout, so the *effective weighted sample count* still grows sublinearly in `H`. That is the quantity this control requires recording, and it is not yet measured.
...
The eventual resume entry should draw from two completed outcomes: **demonstrated visual-control learning at a measured scale** and **a measured horizon–performance–compute result**. Record actual task returns, data budgets, parameter counts, runtime, and variability before writing accomplishment claims. Of those, only the parameter count exists: **686,846 trainable, measured** ([`results/m8/`](../results/m8/)). Returns, budgets consumed, runtime and variability are all still `unmeasured`. Strong implementation and a careful experiment are sufficient; architectural novelty is not a requirement.
